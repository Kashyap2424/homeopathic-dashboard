...
// Hooks Imports
import { useAddProcedureByAdmin } from "../../../hooks/admins/useAdmins";
import { useCreateProcedure } from "../../../hooks/procedures/useProcedures";
...
const PatientsAdd = (props) => {
  ...
  const { isCreateProcedureLoading, createProcedureHandler } =
    useCreateProcedure();
  const { isAddProcedureLoading, addProcedureHandler } =
    useAddProcedureByAdmin();
  ...
  const schema = yup.object().shape({
    ...
    image: yup.mixed().required("Patient image is required"),
  });
  ...
  const onSubmit = async (data) => {
    ...
    if (doctorId) {
      const response = await addProcedureHandler(doctorId, formData);
      if (response.data.status) {
        navigate(`/doctors/${doctorId}`);
        reset();
      }
    } else {
      const response = await createProcedureHandler(formData);
      if (response.data.status) {
          ...
          <div className="gap-4">
            ...
            <div className="flex items-center justify-end-safe my-4">
              ...
              <div className="">
                <LoadingButton
                  type="submit"
                  isLoading={isCreateProcedureLoading || isAddProcedureLoading}
                >
                  Add Patient
                </LoadingButton>
              </div>
